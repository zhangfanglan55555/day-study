<template>
  <div v-bind:class="{ bodyImg: !isBuy && !isLoaded }">
    <div class="video-box topFixed">
      <img
        class="vid"
        :src="lessonDetail.lesson.coverHUrl"
        :class="{ hide: isLoaded }"
      />
      <div
        class="vid-msg"
        :style="{
          display: isBuy && !isHaveLastInfo && !firstPlay ? 'block' : 'none'
        }"
      >
        <div class="msg-txt">
          <p>
            {{
              video.video.videoNum > 9
                ? video.video.videoNum
                : "0" + video.video.videoNum
            }}课时 {{ video.video.videoName }}
          </p>
        </div>
        <a class="msg-btn" href="javascript:;" @click="playVideo">开始学习</a>
      </div>
      <div
        class="vid-msg"
        :style="{ display: isHaveLastInfo ? 'block' : 'none' }"
      >
        <div class="msg-txt">
          <p>
            上次学习到：
            <template v-if="!isBuy && video.video.isTry === 1"
              >[试听]</template
            >{{
              video.video.videoNum > 9
                ? video.video.videoNum
      ...
      <a
        ...
        v-if="isBuy"
        @click="playVideo"
        :style="{ display: isLoaded || isHaveLastInfo ? 'none' : '' }"
      ></a>
    ...
    <div class="tabsBox crse-dtl-tab">
      <div class="topFixed tab-nav">
        <div class="nav-ul">
          <li class="tab-tit">课程介绍</li>
          <li class="tab-tit">课程内容</li>
          <li class="tab-tit">
            评论
        ...
        <div class="panel-item">
          <div class="crse-intro">
            ...
            <div class="grayer txtbox">
              <p>
                {{
                  ~~lessonDetail.lesson.videoCount
                    ? lessonDetail.lesson.videoCount
                    : 0
                }}节课 |
                {{
                  ~~lessonDetail.lesson.favoriteUserNum
                    ? lessonDetail.lesson.favoriteUserNum
                    : 0
                }}人已收藏
              </p>
            ...
            <div class="ycbbox" v-if="!isBuy">
              <price-info
                :lessonPrice="lessonDetail.lesson.lessonPrice"
                :lessonCost="lessonDetail.lesson.lessonCost"
                :isShowSale="true"
              ></price-info>
            </div>
          </div>
          <div class="crse-intro">
            <h3 class="crse-h3tit">课程介绍</h3>
            <div class="txtbox">
            ...
            <template v-if="lessonDetail.lesson.lessonType">
              ...
              <div class="txtbox">
                <p v-if="lessonDetail.lesson.lessonType == 1">
                  此课程购买后永久有效，不限播放次数。
                </p>
                <p v-if="lessonDetail.lesson.lessonType == 2">
                  此课程自购买之日起有效期为1年，有效期内不限播放次数，课程到期后将无法播放。
                </p>
              </div>
            </template>
          </div>
        </div>
        <div class="panel-item crse-content">
          <h3 class="crse-h3tit">课程内容</h3>
          <template v-for="item in videoList">
            <div
              class="yi-flex cont-item"
              @click="changeVideo(item.video)"
              v-bind:class="{
                active: item.video.videoId == vid && isLoaded,
                'cont-lock': !isBuy && !(item.video.isTry === 1),
                icon: !isBuy && !(item.video.isTry === 1)
              }"
            ...
            >
              ...
              <div class="flex1">
                ...
                <p class="time">
                  <span class="grayer"
                    >时长：{{ durationFormat(item.video.videoDuration) }}</span
                  >
                  <template
                    v-if="
                      item.userVideoStatistics &&
                        (isBuy || item.video.isTry === 1)
                    "
                  >
                    <span v-if="item.userVideoStatistics.studyStatus === 1"
                      >播放完成</span
                    >
                    <span v-else-if="item.userVideoStatistics.studyStatus === 2"
                      >已播放{{ item.userVideoStatistics.palyPercent }}%</span
                    >
                    <span v-else>未播放</span>
                  </template>
                </p>
              </div>
              <div class="primary state">
                {{ !isBuy && item.video.isTry === 1 ? "可试听" : "" }}
              </div>
            </div>
          </template>
        </div>
        <div class="panel-item yi-comment" ref="commentArea">
          <scroller :next="next" ref="scroller">
            ...
            <template v-if="hotCommentList.comments.length > 0">
              ...
              <div class="cmt-list ">
                ...
                >
                  <div class="yi-flex cmt-hd center ">
                    <img class="face " :src="item.userInfo.userLogo" />
                    <div class="flex1 ">
                  ...
                  <div class="cmt-bd">
                    <p v-html="formatComment(item.comment.content)"></p>
                  </div>
                  <div class="cmt-ft ">
                    <span
                      class="icon cmt-like "
                      v-if="item.comment.thumNum > 0"
                      v-bind:class="{
                        active: thumupList.indexOf(item.comment.commentId) >= 0
                      }"
                      @click="thumbsup(item.comment.commentId)"
                      >{{ formatNum(item.comment.thumNum) }}</span
                    >
                    <span
                      class="icon cmt-like"
                      v-else
                    ...
                    >
                    <span class="grayer">{{
                      commentTimeFormat(item.comment.createTime)
                    }}</span>
                  </div>
                </div>
              </div>
            ...
            <div
              class="cmt-small-tit"
              id="titTop"
              :style="{ display: commentList.total > 0 ? '' : 'none' }"
            >
              全部评论
            </div>
            <div class="cmt-list">
    ...
    <div class="bottomFixed operation">
      <router-link class="icon ops-home" :to="{ name: 'home' }"></router-link>
      <div
        class="icon ops-coll active"
        v-if="isFav"
        v-on:click="favorite(2)"
      ></div>
      <div class="icon ops-coll" v-else v-on:click="favorite(1)"></div>
      <div
        class="flex1 cmt-btn"
        :style="{ display: !isBuy ? 'none' : 'block' }"
        v-bind:class="{ hide: !isBuy }"
      >
        我来说两句…
      </div>
      <a
        class="yi-btn bg-primary round flex1 ops-btn"
        href="javascript:;"
        v-if="!isBuy"
        @click="lessonBuy"
      >
        <template v-if="lessonDetail.lesson.lessonPrice > 0"
          >马上兑换</template
        >
        <template v-else
          >免费学习</template
    ...
    <div class="bottomFixed cmt-txta">
      <div class="txta-hd">
        <a class="yi-fr release" href="javascript:;" @click="addComment"
          >发布</a
        >
      ...
      <div class="txta-bd">
        ...
        ></textarea>
        <div class="grayer txta-num"><span class="cur">0</span>/500</div>
      </div>
    </div>
  </div>
</template>
...
import { alert, tip } from "_c/common/dialog";
...
export default {
  ...
  methods: {
    ...
    formatComment(content) {
      return this.htmlEncode(content).replace(/\n/g, "<br>");
    },
    initPanel() {
    ...
    limitHandle() {
      if (new Date() - this.lastHandleTime < 500) {
        alert.show("操作太频繁了~~");
        return false;
      }
    ...
    goTest() {
      this.$router.push({ path: "/lesson/testtest", query: { id: this.id } });
    },
    lessonBuy() {
    ...
    initVideoCallback() {
      ...
      this.lessonVideo.oncanplay = function() {
      ...
      this.lessonVideo.onloadeddata = function() {
        ...
        // setTimeout(function() {
        //   _this.lessonVideo.play();
        //   console.log("begin play loaddata");
        // }, 20);
      };
    ...
    favorite(isfav) {
      ...
      post(learn.favorite, { lessonId: this.id, isFavorite: isfav })
        .then(value => {
          tip.show(this.isFav ? "收藏成功" : "已取消收藏");
        })
        .catch(des => {
          // todo
    ...
    next() {
      //  已经加载完
      if (
        this.commentList.total <= this.commentList.comments.length &&
        this.pageIndex > 0
      ) {
        return;
      }
      ...
      this.initCommentList(
        ...
        len => {
          if (
            ...
            //this.pageIndex--;
            this.$nextTick(() => {
              if (this.commentList.comments.length === 0) {
    ...
    addComment() {
      //  如果评论操作还没完成时，不能重复提交
      if (this.commentIsAdding) {
        return;
      }
      this.commentIsAdding = true;
      // 初步验证
      if (
        ...
        // todo 错误提示
        console.log("null " + this.comment_content);
        return;
      }
      if (!this.limitHandle()) {
        return;
